        ...
        foreach (self::$commands as $key => $cmd) {
            ...
            if ($cmd) {
                ...
                if ($errorlevel) {
                    ...
                    // noop
                } elseif (preg_match_all('/^(\S+)\s+(.+)$/m', implode(PHP_EOL, $output), $m)) {
                    // collect metrics
                    foreach ($m[1] as $k => $v) {
    ...
    public static function exportData()
    {
        if (empty(self::$export)) {
            ...
            if (self::$export->export(self::$items, self::$store)) {
                ...
                error_log(self::$export->error ?: 'error exporting data');
                return false;
            }
        } else {
            error_log(self::$store->error ?: 'error loading data from datastore');
            return false;
        }
    }
}
